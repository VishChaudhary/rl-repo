...
def run(train_gate, inference_gate, n_training_iterations=1, n_episodes_for_inferencing = 1, save=True, plot=True, noise_file=noise_file):
    ...
    # ---------------------> Save Results <-------------------------
    if save is True:
        sr = SaveResults(inference_env, alg, target_gate_string=f"Train-{str(train_gate)}, Inference-{str(inference_gate)}")
    ...
    if plot is True:
        ...
        plot_data(save_dir, episode_length=alg._episode_history[0].episode_length, figure_title=f"Training on {str(train_gate)}, Inference on {str(inference_gate)}")
        print("Plots Created")
    # --------------------------------------------------------------
