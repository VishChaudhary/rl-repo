...
class GateSynthEnvRLlibHaar(gym.Env):
    ...
    def compute_reward(self, fidelity):
        ...
        print(f'previous fidelity: {self.prev_fidelity}')
        return (-3 * np.log10(x) + np.log10(y)) + (
...
class GateSynthEnvRLlibHaarNoisy(GateSynthEnvRLlibHaar):
    ...
    def get_observation(self):
        ...
        arr1 = np.append([self.compute_fidelity()] +
                         normalized_relaxation_rates +
                         normalized_detuning,
                         self.unitary_to_observation(self.U))

        return np.append(arr1, self.unitary_to_observation(self.U_target))
