...
def load_and_plot(file_path):
    ...
    gamma_magnitude = df['gamma_magnitude'].values
    gamma_phase = df['gamma_phase'].values
    alpha = df['alpha'].values
    current_Haar_num = df['current_Haar_num'].values
    current_step_per_Haar = df['current_step_per_Haar'].values
    fidelity = df['fidelity'].values

    # Create a time vector based on the number of control pulses
    time_steps = np.arange(len(gamma_magnitude))
...
if __name__ == "__main__":
    load_and_plot('2024-10-20_15-52-50_Y/pulse_data.csv')
